...
def is_spawnable_pokemon(pokemon_id):
    ...
    # Check if Pokémon exists in data
    ...
    # Exclude Mega Pokémon
    if "mega" in pokemon_name:
        return False
    
    # Exclude Dynamax/Gigantamax Pokémon
    if "gmax" in pokemon_name or "gigantamax" in pokemon_name:
        return False
    
    # Exclude custom Pokémon (Shadow Pokémon)
    if pokemon_name.startswith("shadow-"):
        return False
    
    # Allow regional forms to spawn (Alolan, Galarian, Hisuian, Paldean)
    # Only exclude Mega, Gmax, and custom Shadow Pokémon
...
# Filter the rarity lists to exclude non-spawnable Pokémon
...
class SpawnSystem(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if self.server_message_count[server_id] >= self.messages_until_spawn:
            ...
            if redirect_channels:
                print(f"🎯 Using redirect channels for server {server_id}: {redirect_channels}")
                for channel_id in redirect_channels:
                    ...
                    if channel and channel.id not in self.active_spawns:
                        ...
                        if perms.send_messages and perms.attach_files:
                            ...
                            print(f"❌ No permissions in redirect channel: {channel.name}")
            else:
                print(f"🌐 No redirect channels, using default channels for {server_id}")
    ...
    async def spawn_pokemon(self, channel):
        """Spawn a random wild Pokémon"""
        try:
            # Clear any existing spawn in this channel first
            if channel.id in self.active_spawns:
                print(f"🧹 Clearing old spawn in {channel.name}")
                del self.active_spawns[channel.id]
            
            # Check if forest event is active in this server
            ...
            if forest_cog and forest_cog.should_spawn_forest_pokemon():
                ...
                print(f"🌳 Forest event spawning {pokemon_name}")
            else:
                # Get random regular Pokémon from the data
                pokemon_id, rarity = get_random_pokemon_id()
                pokemon_data = POKEMON_DATA[str(pokemon_id)]
                print(f"🌿 Regular spawn: {pokemon_data['name']}")
            ...
            if spawn_image:
                ...
                # Check if this is a forest Pokemon
                is_forest_pokemon = forest_cog and forest_cog.should_spawn_forest_pokemon()
                
                if is_forest_pokemon:
                    embed = discord.Embed(
                        title="🌿 A wild Forest Pokemon has appeared!",
                        description=f"Guess the pokémon and type `@Botachu#1166 catch <pokémon>` or use `/catch <pokémon>` to catch it!",
                        color=discord.Color.green()
                    )
                    embed.set_footer(text="🌳 Forest Event • Special Pokemon Spawn")
                else:
                    embed = discord.Embed(
                ...
                self.active_spawns[channel.id] = {
                    ...
                    "source": "forest_event" if is_forest_pokemon else "regular"
                }
                
                print(f"✨ Spawned {pokemon_data['name']} in {channel.name}")
            
        except Exception as e:
            print(f"Error spawning Pokémon: {e}")

    async def create_spawn_image(self, pokemon_data):
        """Create the spawn image by combining background and Pokémon sprite"""
        try:
            # Paths
            ...
            if not os.path.exists(pokemon_sprite_path):
                print(f"Pokémon sprite not found: {pokemon_sprite_path}")
                return None
    ...
    async def handle_catch_attempt(self, message):
        """Handle when someone tries to catch a Pokémon"""
        ...
        if not (content.startswith(bot_mention) or content.startswith(f"@pokétwo")):
            return
        ...
        try:
            ...
            #correct_name = spawn_data["pokemon_name"].lower()
    ...
    async def successful_catch(self, message, spawn_data):
        """Handle successful Pokémon catch"""
        try:
            user_id = message.author.id
            ...
            # Check for shiny hunt bonus
            shiny_hunt_cog = self.bot.get_cog("ShinyHunt")
            bonus_chance = 0.0
            if shiny_hunt_cog:
                bonus_chance = await shiny_hunt_cog.get_shiny_hunt_bonus(user_id, original_name)
            
            # Calculate shiny chance with bonus
            base_chance = 1/4096  # Base shiny chance
            total_chance = base_chance + (bonus_chance / 100)
            shiny = random.random() < total_chance
            ...
            print(f"✅ Saved Pokémon for user {user_id}")

            # Calculate IV percentage
            total_iv = sum(ivs.values())
            iv_percentage = round((total_iv / 186) * 100, 2)

            success_message = f"🎉 {message.author.mention}, you caught a Level {level} **{display_name}** ({iv_percentage}%)!"
            if shiny:
                success_message += " ✨"
            ...
            total_coins = 25
            reward_messages.append("💰 Pokemon caught! +25 coins")
            ...
            if is_new_species:
                total_coins += 35
                reward_messages.append("📘 First time catch! Added to Pokédex. +35 coins")
            ...
            if total_species_caught in milestones:
                ...
                reward_messages.append(f"🏆 You've caught **{display_name}** {total_species_caught} times! +{milestone_reward} coins")

            # Always update coins since every catch gives rewards
            try:
                print(f"🪙 DEBUG: Updating {total_coins} coins for user {user_id}")
                await profile_collection.update_one(
                    {"user_id": user_id},
                    {"$inc": {"coins": total_coins}},
                    upsert=True
                )
                print(f"✅ DEBUG: Successfully updated coins for user {user_id}")
            except Exception as coin_error:
                print(f"❌ DEBUG: Error updating coins: {coin_error}")
            ...
            # Update shiny hunt progress only for target Pokemon
            try:
                shiny_hunt_cog = self.bot.get_cog("ShinyHunt")
                if shiny_hunt_cog:
                    await shiny_hunt_cog.update_shiny_hunt_progress(user_id, original_name)
            except Exception as e:
                print(f"Error updating shiny hunt progress: {e}")
    ...
    async def force_spawn(self, ctx):
        """Force spawn a Pokémon (admin only)"""
        await self.spawn_pokemon(ctx.channel)
        await ctx.message.delete()  # Delete the command message

    @app_commands.command(name="catch", description="Catch a wild Pokémon!")
    @app_commands.describe(pokemon="The name of the Pokémon to catch")
    async def slash_catch(self, interaction: discord.Interaction, pokemon: str):
        """Catch a wild Pokémon using slash command"""
        channel_id = interaction.channel.id

        if channel_id not in self.active_spawns:
            return await interaction.response.send_message("❌ There are no wild Pokémon in this channel!", ephemeral=True)

        if channel_id in self.catch_locks:
            return await interaction.response.send_message("❌ Someone else is already trying to catch this Pokémon!", ephemeral=True)
        ...
        try:
            ...
            if guess_canonical == correct_canonical:
                ...
                await interaction.response.send_message("🎉 Processing your catch...", ephemeral=True)
            else:
                await interaction.response.send_message("❌ That's not the right Pokémon!", ephemeral=True)

        except Exception as e:
            print(f"Error in slash_catch: {e}")
            await interaction.response.send_message("⚠️ An error occurred while trying to catch the Pokémon.", ephemeral=True)


    @commands.command(name="catch", aliases=["c"])
    async def prefix_catch(self, ctx, *, pokemon: str):
        """Catch a wild Pokémon using prefix command"""
        channel_id = ctx.channel.id

        if channel_id not in self.active_spawns:
            return await ctx.send("❌ There are no wild Pokémon in this channel!")

        if channel_id in self.catch_locks:
            return await ctx.send("❌ Someone else is already trying to catch this Pokémon!")
        ...
        try:
            ...
            if guess_canonical == correct_canonical:
                ...
                await ctx.send(f"❌ That's not the right Pokémon! The correct answer was **{spawn_data['pokemon_name']}**.")

        except Exception as e:
            print(f"Error in prefix_catch: {e}")
            await ctx.send("⚠️ Something went wrong while trying to catch the Pokémon.")

    

    @app_commands.command(name="sai", description="Add a channel where Pokémon can spawn.")
    ...
    async def redirect(self, interaction: discord.Interaction, action: str, channel: discord.TextChannel = None):
        ...
        if action == "list":
            ...
            if server_id in self.redirect_channels and self.redirect_channels[server_id]:
                ...
                if channel_mentions:
                    embed = discord.Embed(
                        title="🎯 Current Spawn Channels",
                        description=f"Pokémon will spawn in these channels:\n{chr(10).join(channel_mentions)}",
                ...
                embed = discord.Embed(
                    title="🎯 Current Spawn Channels",
                    description="No redirect channels set. Pokémon can spawn in any channel.",
                    color=discord.Color.green()
                )
        ...
        if action == "clear":
            ...
            embed = discord.Embed(
                ...
                description="Pokémon can now spawn in any channel again.",
                color=discord.Color.green()
            )
            await interaction.response.send_message(embed=embed)
            return
        ...
        if not channel:
            await interaction.response.send_message("❌ Please provide a channel for add/remove actions!", ephemeral=True)
            return
        ...
        if action == "add":
            ...
            if not (permissions.send_messages and permissions.attach_files):
                await interaction.response.send_message(f"❌ I don't have permissions to send messages and files in {channel.mention}!", ephemeral=True)
                return
            
            if channel.id in self.redirect_channels[server_id]:
                await interaction.response.send_message(f"❌ {channel.mention} is already in the spawn channel list!", ephemeral=True)
                return
            
            self.redirect_channels[server_id].add(channel.id)
            embed = discord.Embed(
                title="✅ Spawn Channel Added",
                description=f"Added {channel.mention} to the spawn channel list.\nPokémon will now spawn in designated channels only.",
                color=discord.Color.green()
            )
            await interaction.response.send_message(embed=embed)
            ...
            if server_id not in self.redirect_channels or channel.id not in self.redirect_channels[server_id]:
                await interaction.response.send_message(f"❌ {channel.mention} is not in the spawn channel list!", ephemeral=True)
                return
    ...
    async def prefix_redirect(self, ctx, action=None, channel: discord.TextChannel = None):
        ...
        if not action:
            ...
            await ctx.send(f"❌ Please specify an action: `add`, `remove`, `list`, or `clear`\nExample: `{prefix}redirect add #channel`")
            return
        ...
        if not channel:
            prefix = ctx.bot.get_primary_prefix()
            await ctx.send(f"❌ Please provide a channel for add/remove actions!\nExample: `{prefix}redirect add #channel`")
            return
        ...
        if action == "add":
            ...
            if not (permissions.send_messages and permissions.attach_files):
                await ctx.send(f"❌ I don't have permissions to send messages and files in {channel.mention}!")
                return
            
            if channel.id in self.redirect_channels[server_id]:
                await ctx.send(f"❌ {channel.mention} is already in the spawn channel list!")
                return
            ...
            # Remove channel from redirect list
            if server_id not in self.redirect_channels or channel.id not in self.redirect_channels[server_id]:
                await ctx.send(f"❌ {channel.mention} is not in the spawn channel list!")
                return
    ...
    def cleanup_old_spawns(self):
        ...
        for channel_id in expired_channels:
            print(f"🧹 Cleaned up expired spawn in channel {channel_id}")
            del self.active_spawns[channel_id]
